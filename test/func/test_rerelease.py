...
def test_cli_rerelease(function_repo):
    ...
    runner = CliRunner()
    runner.invoke(tags.cli.main, ['release', 'a', '-m', 'a'])
    result = runner.invoke(tags.cli.main, ['release', 'a', '-m', 'a'])
...
def test_cli_rerelease_alias(function_repo):
    ...
    runner.invoke(tags.cli.main, ['release', 'a', '-m', 'a'])
    release_alias = 'alias'
    result = runner.invoke(tags.cli.main, ['release', 'a', '-a', release_alias, '-m', 'a'])
...
def test_cli_rerelease_same_alias(function_repo):
    ...
    function_repo.packages('a')
    runner = CliRunner()
    runner.invoke(tags.cli.main, ['release', 'a', '-m', 'a']).output
    release_alias = 'release-alias'
    runner.invoke(tags.cli.main, ['release', 'a', '-a', release_alias, '-m', 'a']).output
    result = runner.invoke(tags.cli.main, ['release', 'a', '-a', release_alias, '-m', 'a'])
    assert result.exit_code == os.EX_USAGE
    assert 'package hasn’t changed' in result.output
...
def test_cli_rerelease_alias_pkg(function_repo):
    ...
    release_alias = 'release-alias'
    runner.invoke(tags.cli.main, ['release', 'a', '-a', release_alias, '-m', 'a']).output
    result = runner.invoke(tags.cli.main, ['release', 'b', '-a', release_alias, '-m', 'a'])
    assert result.exit_code == os.EX_OK
    assert release_alias in result.output
